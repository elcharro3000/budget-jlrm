...
export function parseAmount(v: string): number { const n = Number(String(v).replace(/[^0-9.-]/g, '')); return isFinite(n) ? n : 0 }
export const COLORS = ['#6366F1','#10B981','#F59E0B','#EF4444','#3B82F6','#06B6D4','#84CC16','#F43F5E','#8B5CF6','#14B8A6','#A855F7','#DC2626']
...
export async function getMXNtoUSD(): Promise<number> {
  try {
    ...
    if (r.ok) { const j = await r.json(); const usdToMxn = j?.rates?.MXN; if (usdToMxn>0) return 1/usdToMxn }
  } catch {}
  try {
